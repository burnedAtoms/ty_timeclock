...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create ty_timeclock-2ae5
  fly apps create ty_timeclock-2ae5-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app ty_timeclock-2ae5
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app ty_timeclock-2ae5-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'ty_timeclock-2ae5-staging' does not match app name in config file 'ty_timeclock-2ae5'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name ty_timeclock-2ae5-db
  fly postgres attach --app ty_timeclock-2ae5 ty_timeclock-2ae5-db

  fly postgres create --name ty_timeclock-2ae5-staging-db
  fly postgres attach --app ty_timeclock-2ae5-staging ty_timeclock-2ae5-staging-db
  ```
